/* ==========================================================
   futures-usdc.js – Premium Futures USD-C Engine
   Features: Hybrid Price, Live Trading, Interactive Graph,
   Auto-Holdings, Wallet Integration
   Style: Futures USDC Theme (Electric Blue Accents)
...
(function() {
    ...
    const CONFIG = {
        ...
        // 👇 CONFIGURATION FOR FUTURES USD-C SECTION 👇
        CURRENT_FILE: 'futures-usdc.js', 
        TARGET_CONTAINER: 'usdc', // Matches HTML ID <div id="usd-c">
        // 👆 ---------------------------- 👆
    ...
    async function initApp() {
        injectStyles();
        renderLoader("Syncing Futures USD-C...");
    ...
    function updatePrice(sym, newPrice) {
        ...
        // DOM Update - UNIQUE IDs for Futures USD-C
        const el = document.getElementById(`fuc-price-${sym}`);
        const card = document.getElementById(`fuc-card-${sym}`);
    ...
    function renderTokenList() {
        ...
        if (State.tokens.length === 0) {
            app.innerHTML = `
                <div class="avx-empty">
                    ...
                    <p>No Futures USD-C pairs available.</p>
                </div>`;
            return;
        }
        ...
        app.innerHTML = State.tokens.map(t => {
            ...
            return `
            <div class="avx-card-fuc" id="fuc-card-${t.symbol}">
                
                <!-- TOP SECTION -->
                <div class="avx-fuc-top">
                    <div class="avx-fuc-icon">
                        ${iconHTML}
                    </div>
                    <div class="avx-fuc-info">
                        <div class="avx-fuc-title">
                            <span class="symbol">${t.symbol}</span>
                            <span class="tag">USD-C</span>
                        </div>
                        <div class="avx-fuc-name">${t.full_name || t.name}</div>
                    </div>
                    <div class="avx-fuc-price-box">
                        <div class="avx-fuc-price" id="fuc-price-${t.symbol}">
                            ${fmtINR(p.current)}
                        </div>
                    </div>
                </div>
                ...
                <!-- ACTIONS (BUY / SELL) -->
                <div class="avx-fuc-actions">
                    <button class="avx-btn-fuc buy" onclick="AVX.openTrade('buy', '${t.symbol}')">
                        Buy / Long
                    </button>
                    <button class="avx-btn-fuc sell" onclick="AVX.openTrade('sell', '${t.symbol}')">
                        Sell / Short
                    </button>
                </div>

                <!-- FOOTER (GRAPH LEFT / INFO RIGHT) -->
                <div class="avx-fuc-footer">
    ...
    function drawChart() {
        ...
        // Chart Styling for Futures USD-C (Electric Blue)
    ...
    /* ---------- PREMIUM STYLES INJECTION (FUTURES USD-C SPECIFIC) ---------- */
    function injectStyles() {
        if(document.getElementById('avx-fuc-css')) return;
        const css = `
            @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&display=swap');
            
            /* FUTURES USD-C THEME VARIABLES (Electric Blue) */
            .avx-card-fuc {
                --fc-bg: #ffffff;
                --fc-text: #0f172a;
                --fc-acc: #2563eb; /* Blue 600 */
                --fc-acc-dark: #1d4ed8;
            }
            ...
            .avx-card-fuc {
                ...
                padding: 20px;
                margin-bottom: 16px;
                box-shadow: 0 10px 15px -3px rgba(37, 99, 235, 0.05), 0 4px 6px -2px rgba(37, 99, 235, 0.03);
            ...
            /* HEADER */
            .avx-fuc-top { display: flex; align-items: center; gap: 14px; margin-bottom: 18px; }
            
            .avx-fuc-icon { 
                width: 48px; height: 48px; border-radius: 14px; 
                background: #eff6ff; border: 1px solid #bfdbfe;
                display: flex; align-items: center; justify-content: center; 
                font-size: 20px; overflow: hidden; color: var(--fc-acc);
            }
            .avx-icon-img { width: 100%; height: 100%; object-fit: cover; }
            
            .avx-fuc-info { flex: 1; }
            .avx-fuc-title { display: flex; align-items: center; gap: 6px; }
            .avx-fuc-title .symbol { font-weight: 800; font-size: 17px; color: #1e293b; }
            .avx-fuc-title .tag { font-size: 9px; font-weight: 800; background: #2563eb; color: #fff; padding: 2px 6px; border-radius: 6px; }
            .avx-fuc-name { font-size: 12px; color: #64748b; font-weight: 500; }

            .avx-fuc-price-box { text-align: right; }
            .avx-fuc-price { font-size: 18px; font-weight: 700; color: #1e293b; transition: color 0.3s; font-family: 'Outfit', monospace; }

            /* ACTIONS */
            .avx-fuc-actions { display: flex; gap: 8px; margin-bottom: 8px; }
            ...
            .avx-btn-fuc.buy { background: linear-gradient(to right, #0f172a, #1e3a8a); box-shadow: 0 4px 10px rgba(30, 58, 138, 0.25); }
            .avx-btn-fuc.sell { background: linear-gradient(to right, #b91c1c, #ef4444); box-shadow: 0 4px 10px rgba(239, 68, 68, 0.25); }
            .avx-btn-fuc:active { opacity: 0.9; transform: scale(0.98); }

            /* FOOTER (MATCHING PREMIUM LAYOUT) */
            .avx-fuc-footer { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #bfdbfe; padding-top: 16px; margin-top: 10px; }
            ...
            .avx-foot-btn:hover { color: #2563eb; background: #eff6ff; }
            .avx-foot-btn svg { width: 18px; height: 18px; stroke-width: 2.5; }

            /* LOADER */
            .avx-spinner-fuc { width: 30px; height: 30px; border: 3px solid #bfdbfe; border-top-color: var(--fc-acc); border-radius: 50%; animation: spin 0.8s infinite linear; margin-bottom: 10px; }
            ...
            .avx-inp-cont input:focus { border-color: var(--fc-acc); }
        ...
        `;
        const s = document.createElement('style'); s.id = 'avx-fuc-css'; s.textContent = css; document.head.appendChild(s);
    }
