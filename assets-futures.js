/* ==========================================================
   assets-futures.js – Premium Exchange Engine (Supabase Linked)
...
(function() {
    ...
    const CONFIG = {
        ...
        CURRENT_FILE: 'assets-futures.js', 
        REFRESH_RATE: 2000, 
        AUTO_CLOSE_SEC: 40,
        TABLES: {
            WALLET: 'user_wallets',
    ...
    if (!supaLib) {
        ...
        document.body.innerHTML = '<div style="color:red;padding:20px;">Error: Supabase SDK not found.</div>';
        return;
    }
    ...
    async function initApp() {
        injectStyles();
        renderLoader("Initializing Premium Market...");
    ...
    function renderLoader(msg) {
        const app = document.getElementById('futures');
    ...
    async function openTrade(type, sym) {
        ...
        if(!m) { buildTradeModal(); m = document.getElementById('avx-trade-modal'); }
        ...
        document.getElementById('avx-price-warning').style.display = 'none';
    ...
    function openInfo(sym) {
        ...
        if(!m) { buildInfoModal(); m = document.getElementById('avx-info-modal'); }
    ...
    function openGraph(sym) {
        ...
        if(!m) { buildGraphModal(); m = document.getElementById('avx-graph-modal'); }
