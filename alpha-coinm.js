/* ==========================================================
   alpha-coinm.js – Premium Alpha Coin-M Engine
   Features: Hybrid Price, Live Trading, Interactive Graph,
   Auto-Holdings, Wallet Integration
   Style: Premium Alpha Theme (Specific to Coin-M)
...
(function() {
    ...
    const CONFIG = {
        ...
        // 👇 CONFIGURATION FOR ALPHA COIN-M SECTION 👇
        CURRENT_FILE: 'alpha-coinm.js', 
        TARGET_CONTAINER: 'alpha-coinm', // Matches HTML ID <div id="coin-m">
        // 👆 ---------------------------- 👆
    ...
    async function initApp() {
        injectStyles();
        renderLoader("Syncing alpha-coinm...");
    ...
    function updatePrice(sym, newPrice) {
        ...
        // DOM Update - UNIQUE IDs for Alpha Coin-M
        const el = document.getElementById(`ac-price-${sym}`);
        const card = document.getElementById(`ac-card-${sym}`);
    ...
    function renderTokenList() {
        ...
        if (State.tokens.length === 0) {
            app.innerHTML = `
                <div class="avx-empty">
                    ...
                    <p>No Alpha Coin-M pairs available.</p>
                </div>`;
            return;
        }
        ...
        app.innerHTML = State.tokens.map(t => {
            ...
            return `
            <div class="avx-card-ac" id="ac-card-${t.symbol}">
                
                <!-- TOP SECTION -->
                <div class="avx-ac-top">
                    <div class="avx-ac-icon">
                        ${iconHTML}
                    </div>
                    <div class="avx-ac-info">
                        <div class="avx-ac-title">
                            <span class="symbol">${t.symbol}</span>
                            <span class="tag">COIN-M</span>
                        </div>
                        <div class="avx-ac-name">${t.full_name || t.name}</div>
                    </div>
                    <div class="avx-ac-price-box">
                        <div class="avx-ac-price" id="ac-price-${t.symbol}">
                            ${fmtINR(p.current)}
                        </div>
                    </div>
                </div>
                ...
                <div class="avx-ac-actions">
                    <button class="avx-btn-ac buy" onclick="AVX.openTrade('buy', '${t.symbol}')">
                        BUY
                    </button>
                    <button class="avx-btn-ac sell" onclick="AVX.openTrade('sell', '${t.symbol}')">
                        SELL
                    </button>
                </div>

                <!-- FOOTER (GRAPH LEFT / INFO RIGHT) -->
                <div class="avx-ac-footer">
    ...
    function drawChart() {
        ...
        // Chart Styling for Alpha Coin-M
    ...
    /* ---------- PREMIUM STYLES INJECTION (ALPHA COIN-M SPECIFIC) ---------- */
    function injectStyles() {
        if(document.getElementById('avx-ac-css')) return;
        const css = `
            @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&display=swap');
            
            /* ALPHA COIN-M THEME VARIABLES */
            .avx-card-ac {
                --ac-bg: #ffffff;
                --ac-text: #0f172a;
                --ac-acc: #334155; /* Deep Slate for Alpha */
            }
            ...
            /* HEADER */
            .avx-ac-top { display: flex; align-items: center; gap: 14px; margin-bottom: 18px; }
            ...
            .avx-ac-info { flex: 1; }
            .avx-ac-title { display: flex; align-items: center; gap: 6px; }
            .avx-ac-title .symbol { font-weight: 800; font-size: 17px; color: #1e293b; }
            .avx-ac-title .tag { font-size: 9px; font-weight: 800; background: #334155; color: #fff; padding: 2px 6px; border-radius: 6px; }
            .avx-ac-name { font-size: 12px; color: #64748b; font-weight: 500; }

            .avx-ac-price-box { text-align: right; }
            .avx-ac-price { font-size: 18px; font-weight: 700; color: #1e293b; transition: color 0.3s; font-family: 'Outfit', monospace; }

            /* ACTIONS */
            .avx-ac-actions { display: flex; gap: 8px; margin-bottom: 8px; }
            ...
            .avx-btn-ac.buy { background: linear-gradient(to right, #0f172a, #1e3a8a); box-shadow: 0 4px 10px rgba(30, 58, 138, 0.25); }
            .avx-btn-ac.sell { background: linear-gradient(to right, #b91c1c, #ef4444); box-shadow: 0 4px 10px rgba(239, 68, 68, 0.25); }
            .avx-btn-ac:active { opacity: 0.9; transform: scale(0.98); }

            /* FOOTER (MATCHING PREMIUM LAYOUT) */
            .avx-ac-footer { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #f1f5f9; padding-top: 16px; margin-top: 10px; }
            ...
            .avx-foot-btn:hover { color: #334155; background: #f1f5f9; }
            .avx-foot-btn svg { width: 18px; height: 18px; stroke-width: 2.5; }

            /* LOADER */
            .avx-spinner-ac { width: 30px; height: 30px; border: 3px solid #e2e8f0; border-top-color: #334155; border-radius: 50%; animation: spin 0.8s infinite linear; margin-bottom: 10px; }
        ...
        `;
        const s = document.createElement('style'); s.id = 'avx-ac-css'; s.textContent = css; document.head.appendChild(s);
    }
