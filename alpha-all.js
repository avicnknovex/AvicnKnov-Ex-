...
(function() {
    ...
    const CONFIG = {
        ...
        // 👇 CONFIGURATION FOR ALPHA SECTION 👇
        CURRENT_FILE: 'alpha-all.js', 
        TARGET_CONTAINER: 'alpha-all', // Matches HTML ID <div id="all">
        // 👆 ---------------------------- 👆
    ...
    async function initApp() {
        injectStyles();
        renderLoader("Syncing Alpha Market...");
    ...
    function updatePrice(sym, newPrice) {
        ...
        // DOM Update - UNIQUE IDs for Alpha
        const el = document.getElementById(`aa-price-${sym}`);
        const card = document.getElementById(`aa-card-${sym}`);
    ...
    function renderTokenList() {
        ...
        app.innerHTML = State.tokens.map(t => {
            ...
            return `
            <div class="avx-card-aa" id="aa-card-${t.symbol}">
                
                <!-- TOP SECTION -->
                <div class="avx-aa-top">
                    <div class="avx-aa-icon">
                        ${iconHTML}
                    </div>
                    <div class="avx-aa-info">
                        <div class="avx-aa-title">
                            <span class="symbol">${t.symbol}</span>
                            <span class="tag">ALPHA</span>
                        </div>
                        <div class="avx-aa-name">${t.full_name || t.name}</div>
                    </div>
                    <div class="avx-aa-price-box">
                        <div class="avx-aa-price" id="aa-price-${t.symbol}">
                            ${fmtINR(p.current)}
                        </div>
                    </div>
                </div>
                ...
                <div class="avx-aa-actions">
                    <button class="avx-btn-aa buy" onclick="AVX.openTrade('buy', '${t.symbol}')">
                        BUY
                    </button>
                    <button class="avx-btn-aa sell" onclick="AVX.openTrade('sell', '${t.symbol}')">
                        SELL
                    </button>
                </div>

                <!-- FOOTER (GRAPH LEFT / INFO RIGHT) -->
                <div class="avx-aa-footer">
    ...
    /* ---------- PREMIUM STYLES INJECTION (ALPHA SPECIFIC) ---------- */
    function injectStyles() {
        if(document.getElementById('avx-aa-css')) return;
        const css = `
            @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&display=swap');
            
            /* ALPHA THEME VARIABLES */
            .avx-card-aa {
                --aa-bg: #ffffff;
                --aa-text: #0f172a;
                --aa-acc: #334155; /* Deep Slate for Alpha */
            }
            ...
            /* HEADER */
            .avx-aa-top { display: flex; align-items: center; gap: 14px; margin-bottom: 18px; }
            ...
            .avx-aa-info { flex: 1; }
            .avx-aa-title { display: flex; align-items: center; gap: 6px; }
            .avx-aa-title .symbol { font-weight: 800; font-size: 17px; color: #1e293b; }
            .avx-aa-title .tag { font-size: 9px; font-weight: 800; background: #334155; color: #fff; padding: 2px 6px; border-radius: 6px; }
            .avx-aa-name { font-size: 12px; color: #64748b; font-weight: 500; }

            .avx-aa-price-box { text-align: right; }
            .avx-aa-price { font-size: 18px; font-weight: 700; color: #1e293b; transition: color 0.3s; font-family: 'Outfit', monospace; }

            /* ACTIONS */
            .avx-aa-actions { display: flex; gap: 8px; margin-bottom: 8px; }
            ...
            .avx-btn-aa.buy { background: linear-gradient(to right, #0f172a, #1e3a8a); box-shadow: 0 4px 10px rgba(30, 58, 138, 0.25); }
            .avx-btn-aa.sell { background: linear-gradient(to right, #b91c1c, #ef4444); box-shadow: 0 4px 10px rgba(239, 68, 68, 0.25); }
            .avx-btn-aa:active { opacity: 0.9; transform: scale(0.98); }

            /* FOOTER (MATCHING PREMIUM LAYOUT) */
            .avx-aa-footer { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #f1f5f9; padding-top: 16px; margin-top: 10px; }
            ...
            /* LOADER */
            .avx-spinner-aa { width: 30px; height: 30px; border: 3px solid #e2e8f0; border-top-color: #334155; border-radius: 50%; animation: spin 0.8s infinite linear; margin-bottom: 10px; }
        ...
        `;
        const s = document.createElement('style'); s.id = 'avx-aa-css'; s.textContent = css; document.head.appendChild(s);
    }
