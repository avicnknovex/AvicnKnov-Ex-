/* ==========================================================
   market-spot.js – Premium Spot Market Engine
...
(function() {
    ...
    const CONFIG = {
        ...
        // 👇 CONFIGURATION FOR SPOT SECTION 👇
        CURRENT_FILE: 'market-spot.js', 
        TARGET_CONTAINER: 'market-spot', // Matches HTML ID <div id="spot">
        // 👆 ---------------------------- 👆
    ...
    async function initApp() {
        injectStyles();
        renderLoader("Syncing Spot Market...");
    ...
    function updatePrice(sym, newPrice) {
        ...
        const el = document.getElementById(`ms-price-${sym}`);
        const card = document.getElementById(`ms-card-${sym}`);
    ...
    function renderLoader(msg) {
        ...
        if (app) app.innerHTML = `
            <div class="avx-loader">
                <div class="avx-spinner-spot"></div>
                <p>${msg}</p>
            </div>`;
    }
    ...
    function renderTokenList() {
        ...
        app.innerHTML = State.tokens.map(t => {
            ...
            return `
            <div class="avx-card-ms" id="ms-card-${t.symbol}">
                
                <!-- TOP SECTION -->
                <div class="avx-ms-top">
                    <div class="avx-ms-icon">
                        ${iconHTML}
                    </div>
                    <div class="avx-ms-info">
                        <div class="avx-ms-title">
                            <span class="symbol">${t.symbol}</span>
                            <span class="tag">SPOT</span>
                        </div>
                        <div class="avx-ms-name">${t.full_name || t.name}</div>
                    </div>
                    <div class="avx-ms-price-box">
                        <div class="avx-ms-price" id="ms-price-${t.symbol}">
                            ${fmtINR(p.current)}
                        </div>
                    </div>
                </div>
                ...
                <div class="avx-ms-actions">
                    <button class="avx-btn-ms buy" onclick="AVX.openTrade('buy', '${t.symbol}')">
                        BUY
                    </button>
                    <button class="avx-btn-ms sell" onclick="AVX.openTrade('sell', '${t.symbol}')">
                        SELL
                    </button>
                </div>

                <!-- FOOTER (GRAPH LEFT / INFO RIGHT) -->
                <div class="avx-ms-footer">
    ...
    function injectStyles() {
        if(document.getElementById('avx-ms-css')) return;
        const css = `
            ...
            .avx-card-ms {
                --ms-bg: #ffffff;
                --ms-text: #0f172a;
                --ms-buy: #1e3a8a; /* Dark Blue */
                --ms-sell: #dc2626; /* Red */
            }

            /* CARD LAYOUT - Unique to Spot Market */
            .avx-card-ms {
            ...
            /* HEADER */
            .avx-ms-top { display: flex; align-items: center; gap: 14px; margin-bottom: 18px; }
            ...
            .avx-ms-info { flex: 1; }
            .avx-ms-title { display: flex; align-items: center; gap: 6px; }
            .avx-ms-title .symbol { font-weight: 800; font-size: 17px; color: #1e293b; }
            .avx-ms-title .tag { font-size: 9px; font-weight: 800; background: #e2e8f0; color: #475569; padding: 2px 6px; border-radius: 6px; }
            .avx-ms-name { font-size: 12px; color: #64748b; font-weight: 500; }

            .avx-ms-price-box { text-align: right; }
            .avx-ms-price { font-size: 18px; font-weight: 700; color: #1e293b; transition: color 0.3s; font-family: 'Outfit', monospace; }

            /* ACTIONS */
            .avx-ms-actions { display: flex; gap: 8px; margin-bottom: 8px; }
            ...
            .avx-btn-ms.buy { background: linear-gradient(to right, #0f172a, #1e3a8a); box-shadow: 0 4px 10px rgba(30, 58, 138, 0.25); }
            .avx-btn-ms.sell { background: linear-gradient(to right, #b91c1c, #ef4444); box-shadow: 0 4px 10px rgba(239, 68, 68, 0.25); }
            .avx-btn-ms:active { opacity: 0.9; transform: scale(0.98); }

            /* FOOTER (MATCHING PREMIUM LAYOUT) */
            .avx-ms-footer { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #f1f5f9; padding-top: 16px; margin-top: 10px; }
            ...
            /* LOADER */
            .avx-spinner-spot { width: 30px; height: 30px; border: 3px solid #e2e8f0; border-top-color: #1e293b; border-radius: 50%; animation: spin 0.8s infinite linear; margin-bottom: 10px; }
        ...
        `;
        const s = document.createElement('style'); s.id = 'avx-ms-css'; s.textContent = css; document.head.appendChild(s);
    }
