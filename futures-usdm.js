/* ==========================================================
   futures-usdm.js – Premium Futures USD-M Engine
   Features: Hybrid Price, Live Trading, Interactive Graph,
   Auto-Holdings, Wallet Integration
   Style: Futures Specific Theme (Purple/Violet Accents)
...
(function() {
    ...
    const CONFIG = {
        ...
        // 👇 CONFIGURATION FOR FUTURES USD-M SECTION 👇
        CURRENT_FILE: 'futures-usdm.js', 
    ...
    async function initApp() {
        injectStyles();
        renderLoader("Syncing Futures USD-M...");
        ...
        if (!user) {
            renderError("Please Login to Trade Futures");
            return;
        }
    ...
    function updatePrice(sym, newPrice) {
        ...
        // DOM Update - UNIQUE IDs for Futures USD-M
        const el = document.getElementById(`fum-price-${sym}`);
        const card = document.getElementById(`fum-card-${sym}`);
    ...
    function renderTokenList() {
        ...
        if (State.tokens.length === 0) {
            app.innerHTML = `
                <div class="avx-empty">
                    ...
                    <p>No Futures USD-M pairs available.</p>
                </div>`;
            return;
        }
        ...
        app.innerHTML = State.tokens.map(t => {
            ...
            return `
            <div class="avx-card-fum" id="fum-card-${t.symbol}">
                
                <!-- TOP SECTION -->
                <div class="avx-fum-top">
                    <div class="avx-fum-icon">
                        ${iconHTML}
                    </div>
                    <div class="avx-fum-info">
                        <div class="avx-fum-title">
                            <span class="symbol">${t.symbol}</span>
                            <span class="tag">USD-M</span>
                        </div>
                        <div class="avx-fum-name">${t.full_name || t.name}</div>
                    </div>
                    <div class="avx-fum-price-box">
                        <div class="avx-fum-price" id="fum-price-${t.symbol}">
                            ${fmtINR(p.current)}
                        </div>
                    </div>
                </div>
                ...
                <!-- ACTIONS (BUY / SELL) -->
                <div class="avx-fum-actions">
                    <button class="avx-btn-fum buy" onclick="AVX.openTrade('buy', '${t.symbol}')">
                        Buy / Long
                    </button>
                    <button class="avx-btn-fum sell" onclick="AVX.openTrade('sell', '${t.symbol}')">
                        Sell / Short
                    </button>
                </div>

                <!-- FOOTER (GRAPH LEFT / INFO RIGHT) -->
                <div class="avx-fum-footer">
    ...
    function drawChart() {
        ...
        // Chart Styling for Futures USD-M (Violet/Purple)
        ...
        if(chartCtx.type === 'line') {
            ctx.beginPath();
            ctx.strokeStyle = '#8b5cf6'; // Violet for Futures
            ctx.lineWidth = 3;
            ctx.lineJoin = 'round';
            slice.forEach((d, i) => {
    ...
    /* ---------- PREMIUM STYLES INJECTION (FUTURES USD-M SPECIFIC) ---------- */
    function injectStyles() {
        if(document.getElementById('avx-fum-css')) return;
        const css = `
            @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&display=swap');
            
            /* FUTURES USD-M THEME VARIABLES (Violet/Purple) */
            .avx-card-fum {
                --fm-bg: #ffffff;
                --fm-text: #0f172a;
                --fm-acc: #8b5cf6; /* Violet 500 */
                --fm-acc-dark: #7c3aed;
            }
            ...
            .avx-card-fum {
                ...
                padding: 20px;
                margin-bottom: 16px;
                box-shadow: 0 10px 15px -3px rgba(139, 92, 246, 0.05), 0 4px 6px -2px rgba(139, 92, 246, 0.03);
            ...
            /* HEADER */
            .avx-fum-top { display: flex; align-items: center; gap: 14px; margin-bottom: 18px; }
            
            .avx-fum-icon { 
                width: 48px; height: 48px; border-radius: 14px; 
                background: #f5f3ff; border: 1px solid #ddd6fe;
                display: flex; align-items: center; justify-content: center; 
                font-size: 20px; overflow: hidden; color: var(--fm-acc);
            }
            .avx-icon-img { width: 100%; height: 100%; object-fit: cover; }
            
            .avx-fum-info { flex: 1; }
            .avx-fum-title { display: flex; align-items: center; gap: 6px; }
            .avx-fum-title .symbol { font-weight: 800; font-size: 17px; color: #1e293b; }
            .avx-fum-title .tag { font-size: 9px; font-weight: 800; background: #8b5cf6; color: #fff; padding: 2px 6px; border-radius: 6px; }
            .avx-fum-name { font-size: 12px; color: #64748b; font-weight: 500; }

            .avx-fum-price-box { text-align: right; }
            .avx-fum-price { font-size: 18px; font-weight: 700; color: #1e293b; transition: color 0.3s; font-family: 'Outfit', monospace; }

            /* ACTIONS */
            .avx-fum-actions { display: flex; gap: 8px; margin-bottom: 8px; }
            ...
            .avx-btn-fum.buy { background: linear-gradient(to right, #0f172a, #1e3a8a); box-shadow: 0 4px 10px rgba(30, 58, 138, 0.25); }
            .avx-btn-fum.sell { background: linear-gradient(to right, #b91c1c, #ef4444); box-shadow: 0 4px 10px rgba(239, 68, 68, 0.25); }
            .avx-btn-fum:active { opacity: 0.9; transform: scale(0.98); }

            /* FOOTER (MATCHING PREMIUM LAYOUT) */
            .avx-fum-footer { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #ddd6fe; padding-top: 16px; margin-top: 10px; }
            ...
            .avx-foot-btn:hover { color: #8b5cf6; background: #f5f3ff; }
            .avx-foot-btn svg { width: 18px; height: 18px; stroke-width: 2.5; }

            /* LOADER */
            .avx-spinner-fum { width: 30px; height: 30px; border: 3px solid #ddd6fe; border-top-color: var(--fm-acc); border-radius: 50%; animation: spin 0.8s infinite linear; margin-bottom: 10px; }
            ...
            .avx-inp-cont input:focus { border-color: var(--fm-acc); }
        ...
        `;
        const s = document.createElement('style'); s.id = 'avx-fum-css'; s.textContent = css; document.head.appendChild(s);
    }
