/* ==========================================================
   market-coinm.js – Premium Coin-M Market Engine
   Features: Hybrid Price, Live Trading, Interactive Graph,
   Auto-Holdings, Wallet Integration
   Style: Tech Indigo/Violet Theme (Coin-M Specific)
   Layout: Premium Footer Structure
   ========================================================== */
(function() {
    ...
    const CONFIG = {
        ...
        // 👇 CONFIGURATION FOR COIN-M SECTION 👇
        CURRENT_FILE: 'market-coinm.js', 
        TARGET_CONTAINER: 'coinm', // Matches HTML ID <div id="coin-m">
        // 👆 ---------------------------- 👆
    ...
    async function initApp() {
        ...
        if (!user) {
            renderError("Please Login to Trade coinm");
            return;
        }
    ...
    function updatePrice(sym, newPrice) {
        ...
        // DOM Update - UNIQUE IDs for Coin-M
        const el = document.getElementById(`coinm-price-${sym}`);
        const card = document.getElementById(`coinm-card-${sym}`);
    ...
    function renderLoader(msg) {
        ...
        if (app) app.innerHTML = `
            <div class="avx-loader">
                <div class="avx-spinner-coinm"></div>
                <p>${msg}</p>
            </div>`;
    }
    ...
    function renderTokenList() {
        ...
        app.innerHTML = State.tokens.map(t => {
            ...
            return `
            <div class="avx-card-coinm" id="coinm-card-${t.symbol}">
                
                <div class="avx-coinm-top">
                    <div class="avx-coinm-icon">
                        ${iconHTML}
                    </div>
                    <div class="avx-coinm-info">
                        <div class="avx-coinm-title">
                            <span class="symbol">${t.symbol}</span>
                            <span class="tag">COIN-M</span>
                        </div>
                        <div class="avx-coinm-name">${t.full_name || t.name}</div>
                    </div>
                    <div class="avx-coinm-price-box">
                        <div class="avx-coinm-price" id="coinm-price-${t.symbol}">
                            ${fmtINR(p.current)}
                        </div>
                    </div>
                </div>

                <div class="avx-coinm-actions">
                     <button class="avx-btn-coinm buy" onclick="AVX.openTrade('buy', '${t.symbol}')">
                        Buy / Long
                    </button>
                    <button class="avx-btn-coinm sell" onclick="AVX.openTrade('sell', '${t.symbol}')">
                        Sell / Short
                    </button>
                </div>

                <div class="avx-coinm-footer">
    ...
    async function openTrade(type, sym) {
        ...
        // UI Fix: Badge ko sirf text tag ki tarah use karein
        ...
        // UI Fix: Yahan 'avx-btn-main' class ka hona zaroori hai solid color ke liye
    ...
    function drawChart() {
        ...
        if(chartCtx.type === 'line') {
            ctx.beginPath();
            ctx.strokeStyle = '#6366f1'; // Indigo for Coin-M
            ctx.lineWidth = 3;
            ctx.lineJoin = 'round';
            slice.forEach((d, i) => {
            ...
            gradFill.addColorStop(0, "rgba(99, 102, 241, 0.2)");
    ...
    /* ---------- PREMIUM STYLES INJECTION (COIN-M SPECIFIC) ---------- */
    function injectStyles() {
        if(document.getElementById('avx-coinm-css')) return;
        const css = `
            @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&display=swap');
            
            /* COIN-M THEME VARIABLES (Indigo/Tech) */
            .avx-card-coinm {
                --cm-bg: #ffffff;
                --cm-text: #0f172a;
                --cm-acc: #6366f1; /* Indigo */
                --cm-acc-dark: #4338ca;
            }

            /* CARD LAYOUT - Unique to Coin-M */
            .avx-card-coinm {
            ...
            .avx-card-coinm:hover {
                transform: translateY(-2px);
                box-shadow: 0 20px 25px -5px rgba(99, 102, 241, 0.1), 0 10px 10px -5px rgba(99, 102, 241, 0.04);
            }

            /* HEADER */
            .avx-coinm-top { display: flex; align-items: center; gap: 14px; margin-bottom: 18px; }
            
            .avx-coinm-icon { 
                width: 48px; height: 48px; border-radius: 14px; 
                background: #eef2ff; border: 1px solid #e0e7ff;
                display: flex; align-items: center; justify-content: center; 
                font-size: 20px; overflow: hidden; color: var(--cm-acc);
            }
            .avx-icon-img { width: 100%; height: 100%; object-fit: cover; }
            
            .avx-coinm-info { flex: 1; }
            .avx-coinm-title { display: flex; align-items: center; gap: 6px; }
            .avx-coinm-title .symbol { font-weight: 800; font-size: 17px; color: #1e293b; }
            .avx-coinm-title .tag { font-size: 9px; font-weight: 800; background: #6366f1; color: #fff; padding: 2px 6px; border-radius: 6px; }
            .avx-coinm-name { font-size: 12px; color: #64748b; font-weight: 500; }

            .avx-coinm-price-box { text-align: right; }
            .avx-coinm-price { font-size: 18px; font-weight: 700; color: #1e293b; transition: color 0.3s; font-family: 'Outfit', monospace; }

            /* ACTIONS */
            .avx-coinm-actions { display: flex; gap: 8px; margin-bottom: 8px; }
            ...
            .avx-coinm-footer { display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #e0e7ff; padding-top: 16px; margin-top: 10px; }
            ...
            .avx-foot-btn:hover { color: #6366f1; background: #eef2ff; }
            .avx-foot-btn svg { width: 18px; height: 18px; stroke-width: 2.5; }

            /* LOADER */
            .avx-spinner-coinm { width: 30px; height: 30px; border: 3px solid #e0e7ff; border-top-color: var(--cm-acc); border-radius: 50%; animation: spin 0.8s infinite linear; margin-bottom: 10px; }
            ...
            .avx-inp-cont input:focus { border-color: var(--cm-acc); }
            
            /* UI Fix: Force solid colors on confirm buttons */
            .avx-btn-main { width: 100%; padding: 16px; border: none; border-radius: 14px; font-weight: 700; font-size: 16px; color: white !important; cursor: pointer; }
            .avx-btn-main.buy { background: #6366f1 !important; box-shadow: 0 4px 12px rgba(99, 102, 241, 0.3); }
            .avx-btn-main.sell { background: #f43f5e !important; box-shadow: 0 4px 12px rgba(244, 63, 94, 0.3); }
        ...
        `;
        const s = document.createElement('style'); s.id = 'avx-coinm-css'; s.textContent = css; document.head.appendChild(s);
    }
